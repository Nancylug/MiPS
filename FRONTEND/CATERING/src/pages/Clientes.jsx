...
const Productos = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editandoId) {
        const res = await axios.put(`/productos/${editandoId}`, nuevo);
        setProductos(productos.map(p => p._id === editandoId ? res.data : p));
        mostrarModal('Producto actualizado con éxito.');
        setEditandoId(null);
      } else {
        const res = await axios.post('/productos', nuevo);
        setProductos([...productos, res.data]);
        mostrarModal('Producto guardado con éxito.');
      }
  ...
  const handleEliminar = async (id) => {
    if (window.confirm('¿Estás seguro de eliminar este producto?')) {
      try {
        ...
        mostrarModal('Producto eliminado con éxito.');
      } catch (err) {
  ...
  const generarPDF = () => {
    ...
    img.onload = () => {
      ...
      autoTable(doc, {
        startY: 45,
        head: [['Nombre', 'Descripción', 'Precio', 'Categoría']],
  ...
  return (
    <div className="container my-4">
      <h2 className="text-center text-md-start">Productos</h2>

      {!soloLectura && (
        <form onSubmit={handleSubmit} className="mb-4">
          <div className="row">
            {['nombre', 'descripcion', 'precio', 'categoria'].map((campo) => (
              <div className="col-12 col-md-6 mb-3" key={campo}>
                <label className="form-label">
                  {campo.charAt(0).toUpperCase() + campo.slice(1)}
                </label>
      ...
      <h4 className="text-center text-md-start">Listado de Productos</h4>
      <div className="table-responsive">
        <table className="table table-striped table-bordered">
          <thead className="table-dark">
            <tr>
